using System;
using System.Collections.Generic;

namespace D2SLib.Model.Data;

public partial class Hireling
{
    public string? Hireling1 { get; set; }

    public string? SubType { get; set; }

    public long? Version { get; set; }

    public long? Id { get; set; }

    public long? Class { get; set; }

    public long? Act { get; set; }

    public long? Difficulty { get; set; }

    public long? Level { get; set; }

    public long? Seller { get; set; }

    public string? NameFirst { get; set; }

    public string? NameLast { get; set; }

    public long? Gold { get; set; }

    public long? ExpLvl { get; set; }

    public long? Hp { get; set; }

    public long? HpLvl { get; set; }

    public long? Defense { get; set; }

    public long? DefLvl { get; set; }

    public long? Str { get; set; }

    public long? StrLvl { get; set; }

    public long? Dex { get; set; }

    public long? DexLvl { get; set; }

    public long? Ar { get; set; }

    public long? ArLvl { get; set; }

    public long? DmgMin { get; set; }

    public long? DmgMax { get; set; }

    public long? DmgLvl { get; set; }

    public long? ResistFire { get; set; }

    public long? ResistFireLvl { get; set; }

    public long? ResistCold { get; set; }

    public long? ResistColdLvl { get; set; }

    public long? ResistLightning { get; set; }

    public long? ResistLightningLvl { get; set; }

    public long? ResistPoison { get; set; }

    public long? ResistPoisonLvl { get; set; }

    public string? HireDesc { get; set; }

    public long? DefaultChance { get; set; }

    public string? Skill1 { get; set; }

    public long? Mode1 { get; set; }

    public long? Chance1 { get; set; }

    public string? ChancePerLvl1 { get; set; }

    public long? Level1 { get; set; }

    public long? LvlPerLvl1 { get; set; }

    public string? Skill2 { get; set; }

    public long? Mode2 { get; set; }

    public long? Chance2 { get; set; }

    public string? ChancePerLvl2 { get; set; }

    public long? Level2 { get; set; }

    public long? LvlPerLvl2 { get; set; }

    public string? Skill3 { get; set; }

    public string? Mode3 { get; set; }

    public string? Chance3 { get; set; }

    public string? ChancePerLvl3 { get; set; }

    public string? Level3 { get; set; }

    public string? LvlPerLvl3 { get; set; }

    public string? Skill4 { get; set; }

    public string? Mode4 { get; set; }

    public string? Chance4 { get; set; }

    public string? ChancePerLvl4 { get; set; }

    public string? Level4 { get; set; }

    public string? LvlPerLvl4 { get; set; }

    public string? Skill5 { get; set; }

    public string? Mode5 { get; set; }

    public string? Chance5 { get; set; }

    public string? ChancePerLvl5 { get; set; }

    public string? Level5 { get; set; }

    public string? LvlPerLvl5 { get; set; }

    public string? Skill6 { get; set; }

    public string? Mode6 { get; set; }

    public string? Chance6 { get; set; }

    public string? ChancePerLvl6 { get; set; }

    public string? Level6 { get; set; }

    public string? LvlPerLvl6 { get; set; }

    public long? HiringMaxLevelDifference { get; set; }

    public long? Resurrectcostmultiplier { get; set; }

    public long? Resurrectcostdivisor { get; set; }

    public long? Resurrectcostmax { get; set; }

    public string? Equivalentcharclass { get; set; }
}
