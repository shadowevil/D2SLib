using System;
using System.Collections.Generic;

namespace D2SLib.Model.Data;

public partial class Rune
{
    public string? Name { get; set; }

    public string? RuneName { get; set; }

    public long? Complete { get; set; }

    public long? FirstLadderSeason { get; set; }

    public long? LastLadderSeason { get; set; }

    public long? PatchRelease { get; set; }

    public string? Itype1 { get; set; }

    public string? Itype2 { get; set; }

    public string? Itype3 { get; set; }

    public long? Itype4 { get; set; }

    public long? Itype5 { get; set; }

    public long? Itype6 { get; set; }

    public long? Etype1 { get; set; }

    public long? Etype2 { get; set; }

    public long? Etype3 { get; set; }

    public string? RunesUsed { get; set; }

    public string? Rune1 { get; set; }

    public string? Rune2 { get; set; }

    public string? Rune3 { get; set; }

    public string? Rune4 { get; set; }

    public string? Rune5 { get; set; }

    public string? Rune6 { get; set; }

    public string? T1code1 { get; set; }

    public string? T1param1 { get; set; }

    public long? T1min1 { get; set; }

    public long? T1max1 { get; set; }

    public string? T1code2 { get; set; }

    public string? T1param2 { get; set; }

    public long? T1min2 { get; set; }

    public long? T1max2 { get; set; }

    public string? T1code3 { get; set; }

    public string? T1param3 { get; set; }

    public long? T1min3 { get; set; }

    public long? T1max3 { get; set; }

    public string? T1code4 { get; set; }

    public string? T1param4 { get; set; }

    public long? T1min4 { get; set; }

    public long? T1max4 { get; set; }

    public string? T1code5 { get; set; }

    public string? T1param5 { get; set; }

    public long? T1min5 { get; set; }

    public long? T1max5 { get; set; }

    public string? T1code6 { get; set; }

    public string? T1param6 { get; set; }

    public long? T1min6 { get; set; }

    public long? T1max6 { get; set; }

    public string? T1code7 { get; set; }

    public string? T1param7 { get; set; }

    public long? T1min7 { get; set; }

    public long? T1max7 { get; set; }

    public string? Eol { get; set; }
}
